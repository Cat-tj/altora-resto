...
- **ALT-DEF-010/ALT-DEF-014 (composite tenant/outlet-scoped FK, lihat ADR-013 di `docs/engineering/DECISION-LOG.md`):** `GILIRAN_KASIR.outletId` dan `PEMBAYARAN.outletId` kini composite-FK `(tenantId, outletId) -> Outlet(tenantId, id)`; `PEMBAYARAN.pesananId` kini composite-FK `(tenantId, pesananId) -> Pesanan(tenantId, id)`. Model `AlokasiPembayaran` terpisah (split bill per pembayaran) yang diminta `ALT-DEF-014` **belum** ditambahkan pada batch ini - batch ini hanya memperbaiki tenant/outlet-safety pada model `Pembayaran` yang sudah ada; rework model split bill itu sendiri di luar cakupan (lihat `docs/engineering/DEFECT-LEDGER.md` ALT-DEF-014, tetap `SIAP_DIVERIFIKASI` bukan `DITUTUP`).
