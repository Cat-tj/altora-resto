...
- **ALT-DEF-010 (composite tenant/outlet-scoped FK, lihat ADR-013 di `docs/engineering/DECISION-LOG.md`):** `GUDANG.outletId` kini composite-FK `(tenantId, outletId) -> Outlet(tenantId, id)`. `STOK_BAHAN` sebelumnya TIDAK punya `tenantId` sama sekali (gap ditemukan saat audit) - kini ditambahkan `tenantId` + DUA composite-FK ke `Gudang(tenantId, id)` dan `Bahan(tenantId, id)`. `MUTASI_STOK.gudangId` sebelumnya ada sebagai kolom TANPA relasi FK sama sekali (gap ditemukan saat audit) - kini ditambahkan sebagai composite-FK langsung ke `Gudang(tenantId, id)`; `MUTASI_STOK.bahanId` juga di-upgrade ke composite-FK `(tenantId, bahanId) -> Bahan(tenantId, id)`. `STOK_OPNAME.gudangId` juga composite-FK ke `Gudang(tenantId, id)`.
