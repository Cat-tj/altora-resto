...
- **ADR-033:** `RekapKasHarian.diverifikasiOlehId` (nullable) dan `BiayaOperasional.dicatatOlehId` dipindah dari FK langsung ke `Pengguna` menjadi composite-FK OUTLET-LEVEL `(tenantId, outletId, xxxOlehId) -> KeanggotaanOutlet(tenantId, outletId, id)`. Lihat `docs/engineering/DECISION-LOG.md` ADR-033.
