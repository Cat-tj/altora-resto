...
erDiagram
    ...
    TIKET_DAPUR {
        ...
        string alasanPembatalan "nullable, WAJIB ketika status=DIBATALKAN (CHECK constraint DB, ADR-036 sub-problem D)"
    }
    TIKET_DAPUR_BARIS {
        string id PK
...
- **ADR-036 sub-problem D:** `alasanPembatalan` WAJIB diisi begitu `status = DIBATALKAN` - ditegakkan CHECK constraint database `tiket_dapur_alasan_wajib_saat_dibatalkan` (bukan hanya validasi aplikasi). Ini prasyarat agar predikat "order ready" (`NOT EXISTS` tiket aktif yang bukan `SIAP`/`DISAJIKAN`/`DIBATALKAN` - lihat SQL persis di `STATE-MACHINES.md` bagian 5) bisa mengecualikan tiket `DIBATALKAN` dari guard kesiapan pesanan TANPA kehilangan jejak audit kenapa tiket itu dikecualikan.
...
- **ADR-033:** `RiwayatStatusTiketDapur.diubahOlehId` (nullable) dan `GelombangDapur.dipicuOlehId` (nullable) dipindah dari FK langsung ke `Pengguna` menjadi composite-FK TENANT-LEVEL `(tenantId, xxxOlehId) -> KeanggotaanTenant(tenantId, id)`. Lihat `docs/engineering/DECISION-LOG.md` ADR-033.
