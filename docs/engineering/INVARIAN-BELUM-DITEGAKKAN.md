...
- **Kategori C (dijaga transaksi aplikasi): 12 baris** (INV-015 s.d. INV-024, INV-045, INV-046) — **DIPERBARUI batch ADR-033.** `INV-045`/`INV-046` (BARU): dua sub-syarat aktor tenant-scoped yang STRUKTURAL tidak bisa dijamin composite-FK statis — (045) status `AKTIF` keanggotaan SAAT command dijalankan, (046) akses outlet spesifik untuk field tenant-level + evaluasi permission/izin saat command dijalankan. Cross-ref `ALT-DEF-045`, `INV-044`. Tidak satu pun baris di kategori ini memakai optimistic concurrency (kolom versi belum ada di schema) — seluruhnya benar-benar tanpa locking apa pun hari ini kecuali rencana `SELECT ... FOR UPDATE`/atomic `UPDATE ... WHERE` yang juga belum berupa kode.
...
- **Total: 47 baris invariant** (naik dari 46 baris pada versi dokumen sebelumnya) — kenaikan batch ADR-034 berasal dari audit+migrasi field uang: 1 baris kategori A baru (`INV-047`, migrasi 51 field uang `Int`→`BigInt`, ceiling overflow dinaikkan ~9 orde besaran, sudah terpasang dan teruji lewat probe overflow nyata). (Batch sebelumnya, ADR-033, menyumbang 1 baris kategori A + 2 baris kategori C dari audit actor tenant-scoped.)
