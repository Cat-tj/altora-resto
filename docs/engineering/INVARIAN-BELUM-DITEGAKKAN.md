...
- **Kategori A (migrasi resmi): 10 baris** (INV-001 s.d. INV-007, INV-012, INV-013, INV-014) — **DIPERBARUI batch ADR-032.** `INV-006`/`INV-007` desainnya diredesain total (pointer `dibalikOlehId` -> `membalikMutasiId`, append-only jadi unkondisional) dan `INV-012`/`INV-013`/`INV-014` (ledger keanggotaan, sebelumnya B2/ALT-DEF-043) DIPINDAH ke sini — keempatnya sekarang memakai SATU PASANG fungsi trigger generik (`ledger_tolak_ubah`/`ledger_validasi_pembalik`) di migrasi `20260726090000_redesign_ledger_reversal_membalik_pattern`, diverifikasi lewat `ledger-reversal-membalik-invariants.test.ts`. Lihat ADR-032. (Migrasi sebelumnya, `baseline_correction_loop`+`harden_manual_invariants`, tetap menyumbang INV-001 s.d. INV-005 — lihat ADR-031.)
