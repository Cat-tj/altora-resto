...
- **Kategori A (migrasi resmi): 11 baris** (INV-001 s.d. INV-007, INV-012, INV-013, INV-014, INV-044) — **DIPERBARUI batch ADR-033.** `INV-044` (BARU): 47 field aktor tenant-scoped (`*OlehId`/`Notification.keanggotaanTenantId`) dipindah dari FK langsung ke `Pengguna` menjadi composite-FK ke `KeanggotaanTenant`/`KeanggotaanOutlet`, migrasi `20260726100000_actor_tenant_outlet_scoped`, diverifikasi lewat `actor-keanggotaan-tenant-outlet-invariants.test.ts`. Lihat ADR-033. (Batch sebelumnya, ADR-032: `INV-006`/`INV-007` diredesain total dan `INV-012`/`INV-013`/`INV-014` DIPINDAH ke sini, memakai SATU PASANG fungsi trigger generik `ledger_tolak_ubah`/`ledger_validasi_pembalik` di migrasi `20260726090000_redesign_ledger_reversal_membalik_pattern`. Migrasi lebih awal, `baseline_correction_loop`+`harden_manual_invariants`, menyumbang INV-001 s.d. INV-005 — lihat ADR-031.)
...
- **Total: 46 baris invariant** (naik dari 43 baris pada versi dokumen sebelumnya) — kenaikan batch ADR-033 berasal dari audit actor tenant-scoped: 1 baris kategori A (`INV-044`, penegakan composite-FK yang SUDAH terpasang dan teruji) + 2 baris kategori C (`INV-045`/`INV-046`, dua sub-syarat runtime/temporal yang STRUKTURAL tidak bisa ditegakkan FK statis, menunggu batch handler/service-layer terpisah).
