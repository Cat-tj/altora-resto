...
| POST | `/api/v1/pesanan/{id}/promo` | Terapkan kode promo/kupon ke pesanan - menulis `PromoPemakaian` + `PromoPemakaianBaris` + `PromoSnapshot` (ALT-DEF-009/ADR-026). Boleh dipanggil berkali-kali untuk promo BERBEDA (stacking) selama lolos resolusi konflik `stackingPolicy`/`conflictGroup`/`prioritas` (lihat ADR-026 Keputusan 1) - setiap promo berbeda menghasilkan baris `PromoPemakaian` header-nya sendiri. **Promo yang SAMA dan `repeatable=true` (ALT-DEF-038/ADR-038):** memanggil endpoint ini ulang untuk promo yang SUDAH diterapkan pada pesanan ini TIDAK membuat baris `PromoPemakaian` kedua (dicegah `@@unique([pesananId, promoId])`) - service-layer menaikkan `jumlahPenerapan` pada baris header yang SUDAH ADA dan menambahkan baris `PromoPemakaianBaris` baru (bertanda `nomorPenerapan` berikutnya) di bawahnya; "berapa kali promo X sudah efektif diterapkan ke pesanan ini" karena itu dibaca dari SATU baris `PromoPemakaian.jumlahPenerapan`, BUKAN dari `COUNT(*)` baris `PromoPemakaian` untuk promo tersebut. **Wajib header `Idempotency-Key`** (`ALT-PLT-018`, kasus "penerapan promo" di master spec). | `promo.terapkan` (KASIR/PELAYAN, lihat `PERMISSION-MATRIX.md`) - ALT-DEF-034: kode ini menggantikan `promo.kelola` yang dipakai sebelum ADR-026 untuk aksi ini, `promo.kelola` sekarang HANYA untuk CRUD definisi promo (ALT-PRM-001). |
